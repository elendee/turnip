...
switch ($type) {
	...
	case 'tournament':
		$sql = $pdo->prepare('INSERT INTO tournaments ( name, date, description ) VALUES ( ?, ?, ?)');
		$sql->execute([ $post->name, $post->date, $post->description ]);
		$res->success = true;
		break;
	...
	case 'team':
		...
		$sql->execute([ $post->name, $mkey, sql_datetime( time() ) ]);
		$res->success = true;
		break;
	...
	case 'player':
		...
		$sql->execute([ $post->name, $post->surname, $post->email, $post->position, sql_datetime( time() ) ]); 
		$res->success = true;
		break;
	...
	case 'registration':
		try{
			...
			$sql->execute([ $post->team_key, $post->tourney_key, sql_datetime( time() ) ]);
			$res->success = true;
	...
	case 'player-registration':
		try{
			...
			$sql->execute([ $post->team_key, $post->player_key, sql_datetime( time() ) ]);
			$res->success = true;
		}catch( PDOException $e ){
			if(strpos($e->getMessage(), 'Duplicate') > 0 ){
